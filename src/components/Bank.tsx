...
const Bank = ({ updateItem }: bankProps): JSX.Element => {
  const items = [
    {
      name: 'Arrow',
  ...
  return (
    <div style={{ left: leftPosition }} className={styles.bank}>
      <div onClick={toggleBank} className={styles.slider}>
        {toggleIcon}
      </div>
      {items.map((item) => {
        if (item.name === 'Dungeon Marker' || item.name === 'Shop Marker') {
          return (
            <div key={`bank-${item.name}`} onClick={() => updateItem(item)}>
              <Frame
                item={item}
                drag={false}
                handleStop={function (i: number, x: number, y: number): void {
